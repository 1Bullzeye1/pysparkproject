from sparksessionutils import spark
...
df= airdf.fillna('(unknown)',['alias','iata','callsign','icao','country'])
df1 = df.withColumn('alias',translate(df['alias'],r'\N','(unknown)')).show(truncate=False)
...
details = portdf.join(a, portdf.Iata == a.src_airport,'inner').select([portdf.Airport_id,portdf.Name,portdf.City,a.src_airport,a.dest_airport,a['count']]).filter(a['count'] >= b).show()
...
airdet = airdf.join(a, airdf.airline_id == a.airline_id,'inner').select(airdf.airline_id,airdf.name,a.src_airport,a.dest_airport)
airdet.show()